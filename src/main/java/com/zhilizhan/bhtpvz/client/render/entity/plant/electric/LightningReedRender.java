...
@OnlyIn(Dist.CLIENT)
public class LightningReedRender  extends PVZPlantRender<LightningReedEntity> {
   private static final RenderType BEAM_RENDER_TYPE= RenderType.lightning();

    public boolean shouldRender(LightningReedEntity livingEntity, Frustum camera, double camX, double camY, double camZ) {
        if (super.shouldRender(livingEntity, camera, camX, camY, camZ)) {
            return true;
        } else {
            if (livingEntity.hasActiveAttackTarget()) {
                LivingEntity target = livingEntity.getActiveAttackTarget();
                if (target != null) {
                    Vec3 pos2 = this.getPosition(target, (double)target.getBbHeight() * 0.5, 1.0F);
                    Vec3 pos1 = this.getPosition(livingEntity, (double)livingEntity.getEyeHeight(), 1.0F);
                    return camera.isVisible(new AABB(pos1.x, pos1.y, pos1.z, pos2.x, pos2.y, pos2.z));
                }
            }
    ...
    public void render(LightningReedEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int packedLight) {
        ...
        if (target != null) {
            ...
            Vec3 pos2 = this.getPosition(target, (double)target.getBbHeight() * 0.5, partialTicks);
            Vec3 pos1 = this.getPosition(entity, (double)l, partialTicks);
            Vec3 lv4 = pos2.subtract(pos1);
    ...
    public LightningReedRender(EntityRenderDispatcher rendererManager) {
        super(rendererManager, new LightningReedModel(), 0.4F);
    }
}