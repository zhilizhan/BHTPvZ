...
public final class BHTPvZPlants extends PlantType {
    ...
    // Ice Cabbage
    public static final IPlantType ICE_CABBAGE_PULT = new BHTPvZPlants("ice_cabbage_pult", new PlantFeatures().cost(175).requiredLevel(17).cd(CoolDowns.VERY_FAST).rank(RankTypes.BLUE).essence(EssenceTypes.ICE).entityType(BHTPvZEntityTypes.ICE_CABBAGE_PULT::get).summonCard(BHTPvZItems.NUT_BOWLING_CARD).enjoyCard(BHTPvZItems.NUT_BOWLING_ENJOY_CARD).plantModel(() -> IceCabbagePultModel::new).scale(0.95f).commonSkill(Collections.singletonList(SkillTypes.MORE_CABBAGE_DAMAGE)));
